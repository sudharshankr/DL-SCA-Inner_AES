...
def labelize(plaintexts, keys, byte_attacked):
    return v_hamming(AES_Sbox[plaintexts[:, byte_attacked] ^ keys[:, byte_attacked]])
...
in_file = h5py.File("../data/traces/ATMega8515_raw_traces.h5", "r")
...
# do we need points of interest in the first place?
...
out_file = h5py.File("../data/traces/ASCAD_stored_traces.h5", "w")
