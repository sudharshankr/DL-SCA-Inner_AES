import numpy as np
from aeskeyschedule import key_schedule
import progressbar

widgets=[
    ' [', progressbar.Timer(), '] ',
    progressbar.Bar(),
    ' (', progressbar.ETA(), ') ',
]

aes_sbox = np.array([
    
])


hamming_lookup = np.array([0, 1, 1, 2, 1, 2, 2, 3, 1, 2, 2, 3, 2, 3, 3, 4, 1, 2, 2, 3, 2, 3,
       3, 4, 2, 3, 3, 4, 3, 4, 4, 5, 1, 2, 2, 3, 2, 3, 3, 4, 2, 3, 3, 4,
       3, 4, 4, 5, 2, 3, 3, 4, 3, 4, 4, 5, 3, 4, 4, 5, 4, 5, 5, 6, 1, 2,
       2, 3, 2, 3, 3, 4, 2, 3, 3, 4, 3, 4, 4, 5, 2, 3, 3, 4, 3, 4, 4, 5,
       3, 4, 4, 5, 4, 5, 5, 6, 2, 3, 3, 4, 3, 4, 4, 5, 3, 4, 4, 5, 4, 5,
       5, 6, 3, 4, 4, 5, 4, 5, 5, 6, 4, 5, 5, 6, 5, 6, 6, 7, 1, 2, 2, 3,
       2, 3, 3, 4, 2, 3, 3, 4, 3, 4, 4, 5, 2, 3, 3, 4, 3, 4, 4, 5, 3, 4,
       4, 5, 4, 5, 5, 6, 2, 3, 3, 4, 3, 4, 4, 5, 3, 4, 4, 5, 4, 5, 5, 6,
       3, 4, 4, 5, 4, 5, 5, 6, 4, 5, 5, 6, 5, 6, 6, 7, 2, 3, 3, 4, 3, 4,
       4, 5, 3, 4, 4, 5, 4, 5, 5, 6, 3, 4, 4, 5, 4, 5, 5, 6, 4, 5, 5, 6,
       5, 6, 6, 7, 3, 4, 4, 5, 4, 5, 5, 6, 4, 5, 5, 6, 5, 6, 6, 7, 4, 5,
       5, 6, 5, 6, 6, 7, 5, 6, 6, 7, 6, 7, 7, 8])


def galois_mult(a, b):
    """
    Multiplication in the Galois field GF(2^8).
    """
    p = 0
    # hi_bit_set = 0
    for i in range(8):
        if b & 1 == 1:
            p ^= a
        hi_bit_set = a & 0x80
        a <<= 1
        if hi_bit_set == 0x80:
            a ^= 0x1b
        b >>= 1
    return p % 256


def galois_mult_np(a, b):
    """
    Multiplication in the Galois field GF(2^8).
    """
    # p = np.zeros((a.shape[0],))
    p = 0
    # hi_bit_set = 0
    for i in range(8):
        if b & 1 == 1:
            p ^= a
        hi_bit_set = a & 0x80
        a <<= 1
        hi_compare = np.full((hi_bit_set.shape[0],), 0x80)
        idxs = np.where((hi_bit_set == hi_compare) == True)[0]
        # if hi_bit_set == 0x80:
        #     a ^= 0x1b
        a[idxs] ^= 0x1b
        b >>= 1
    return p % 256


def calc_round_key_byte(rnd, idx_byte, keys):
    expanded_key_bytes = np.zeros((keys.shape[0],), dtype="uint8")
    for i in range(keys.shape[0]):
        expanded_key_bytes[i] = key_schedule(keys[i])[rnd][idx_byte]
    return expanded_key_bytes


def hamming(n):
    return bin(int(n)).count("1")


v_hamming = np.vectorize(hamming)   #vectorizing hamming func and galois mult func
v_gmul = np.vectorize(galois_mult)


def write_to_npz(filename, ranks, trace_cnt, key_probs):
    print("Saving file")
    output_file = filename
    np.savez(output_file, ranks=ranks, trace_cnt=trace_cnt, key_probs=key_probs)